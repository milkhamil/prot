...
def bot(op):
    try:
        ...
        if op.type == 25:
            ...
            elif "1cbio " in msg.text:
                string = msg.text.replace("1cbio ","")
                if len(string.decode('utf-8')) <= 500:
                    profile = ki.getProfile()
                    profile.statusMessage = string
                    ki.updateProfile(profile)
                    ki.sendText(msg.to,"Update Bio:\n👉 " + string + "👈")
            elif "2cbio " in msg.text:
                string = msg.text.replace("2cbio ","")
                if len(string.decode('utf-8')) <= 500:
                    profile = ki2.getProfile()
                    profile.statusMessage = string
                    ki2.updateProfile(profile)
                    ki2.sendText(msg.to,"Update Bio:\n👉 " + string + "👈")
            elif "3cbio " in msg.text:
                string = msg.text.replace("3cbio ","")
                if len(string.decode('utf-8')) <= 500:
                    profile = ki3.getProfile()
                    profile.statusMessage = string
                    ki3.updateProfile(profile)
                    ki3.sendText(msg.to,"Update Bio:\n👉 " + string + "👈")
            elif "4cbio " in msg.text:
                string = msg.text.replace("4cbio ","")
                if len(string.decode('utf-8')) <= 500:
                    profile = ki4.getProfile()
                    profile.statusMessage = string
                    ki4.updateProfile(profile)
                    ki4.sendText(msg.to,"Update Bio:\n👉 " + string + "👈")
            elif "5cbio " in msg.text:
                string = msg.text.replace("5cbio ","")
                if len(string.decode('utf-8')) <= 500:
                    profile = ki5.getProfile()
                    profile.statusMessage = string
                    ki5.updateProfile(profile)
                    ki5.sendText(msg.to,"Update Bio:\n👉 " + string + "👈")
            elif "6cbio " in msg.text:
                string = msg.text.replace("6cbio ","")
                if len(string.decode('utf-8')) <= 500:
