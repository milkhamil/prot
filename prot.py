...
cl = LINETCR.LINE()
cl.login(token="EmaqaKL8vBmkq5GXdwN9.hneFXO8OgBj1+YKplCqRoq.d4eSmWr/TG2XNOekh52VUvoNq7DwNcqQcdrQCL0Ty1w=")
cl.loginResult

kc = LINETCR.LINE()
kc.login(token="EmTw94G6h9h3l2kSjNnc.mTnlxsO+mIWobeSYdF8pha./q751+VU9UBeIOrbKX5duGmLgkUnq25qVOOPlAools8=")
kc.loginResult

kk = LINETCR.LINE()
kk.login(token="Emk6OA7h6439Ea5Ge5t7.MD52GENmv7QC+jzY4LrPnW.Iz2FD1cO4gwXM1NFsnvKaTGWXRsy8WPCGHNPhg+KXXo=")
kk.loginResult
...
Bots=["uf2a2e54db6faa9b7da06913c998194e6","ua0829fc2869fcc727d2bd234e788feb6",Bmid,Cmid]
...
#-----------------------fungsi rename----------------------
                ...
                md = ""
                if wait["contact"] == True: md+=" 􀔃􀅁check􏿿Contact\n"
                else: md+=" 􀔃􀄿silang􏿿Contact\n"
                if wait["autoJoin"] == True: md+=" 􀔃􀅁check􏿿Auto join\n"
                else: md +=" 􀔃􀄿silang􏿿Auto join : 􀔃􀄿silang􏿿\n"
                if wait["autoCancel"]["on"] == True:md+=" 􀔃􀅁check􏿿Group cancel :" + str(wait["autoCancel"]["members"]) + "\n"
                else: md+= " 􀔃􀄿silang􏿿Group cancel\n"
                if wait["leaveRoom"] == True: md+=" 􀔃􀅁check􏿿Auto leave\n"
                else: md+=" 􀔃􀄿silang􏿿Auto leave\n"
                if wait["timeline"] == True: md+=" 􀔃􀅁check􏿿Share\n"
                else:md+=" 􀔃􀄿silang􏿿Share\n"
                if wait["autoAdd"] == True: md+=" 􀔃􀅁check􏿿Auto add\n"
                else:md+=" 􀔃􀄿silang􏿿Auto add\n"
                if wait["commentOn"] == True: md+=" 􀔃􀅁check􏿿Comment\n"
                else:md+=" 􀐂􀇰pin􏿿Comment\n"
                if wait["atjointicket"] == True: md+=" 􀔃􀅁check􏿿Auto Join Group by Ticket \n"
                else:md+=" 􀔃􀄿silang􏿿Auto Join Group by Ticket"
                cl.sendText(msg.to,"                􀐂􀇰pin􏿿settings􀐂􀇰pin􏿿\n\n" + md)
...
#---------------------fungsi absen--------------------------
                ...
                cl.sendText(msg.to,"hadir􀜁􀅔Har Har􏿿")
                ki.sendText(msg.to,"hadir􀜁􀅔Har Har􏿿")
                kc.sendText(msg.to,"")
#--------------------fungsi speed---------------------------
            elif msg.text in ["Sp","Speed","speed"]:
                start = time.time()
                cl.sendText(msg.to, "bentar ngitung dulu􀜁􀅔Har Har􏿿")
                cl.sendText(msg.to, "...")
                ...
                if msg.from_ in admin:
					if msg.toType == 2:
						...
						for tag in wait["blacklist"]:
							matched_list+=filter(lambda str: str == tag, gMembMids)
						if matched_list == []:
							cl.sendText(msg.to,"Tidak ada blacklist user")
							ki.sendText(msg.to,"Tidak ada blacklist user")
							kk.sendText(msg.to,"Tidak ada blacklist user")
							kc.sendText(msg.to,"Tidak ada blacklist user")
							return
						for jj in matched_list:
						...
						cl.sendText(msg.to,"Blacklist emang pantas di usir")
						ki.sendText(msg.to,"Blacklist emang pantas di usir")
						kk.sendText(msg.to,"Blacklist emang pantas di usir")
						kc.sendText(msg.to,"Blacklist emang pantas di usir")
#------------------fungsi cancel invite----------
				...
				if msg.from_ in admin:
					if msg.toType == 2:
						...
						cl.sendText(msg.to,"Selesai bos.")
#---------------fungsu random-------
